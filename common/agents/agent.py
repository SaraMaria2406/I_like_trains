...
class Agent(BaseAgent):
    ...
    def __init__(self, nickname: str, network: NetworkManager, logger: str = "client.agent",timeout: float = 1/REFERENCE_TICK_RATE):
        super().__init__(nickname, network, logger, timeout)
        self.wall_buffer = 3  # Look out zone for walls
        self.last_positions = []  # Detection of mouvement loops
        self.max_positions_memory = 10  # Max position to memorize
        self.stuck_threshold = 5  # Number of positions before considering the train stuck
        self.passenger_group_bonus = 2  # Bonus for passenger groups
        self.prev_trains = {}  # To detect collisions with other trains
        self.forced_direction_change = False  # Forced direction change after collision
    ...
    def update_position_history(self):
        ...
        if len(self.last_positions) > self.max_positions_memory:
            self.last_positions.pop(0)  # Deletes the oldest position
    ...
    def is_stuck(self):
        ...
        return len(unique_positions) <= 2  # Considered stuck if alternating between 1-2 positions 
    ...
    def get_passenger_clusters(self):
        ...
        # Converts the passenger's positions in coodinates
        grid_passengers = []
        for p in self.passengers:
            px, py = p["position"]
            grid_passengers.append((px // self.cell_size, py // self.cell_size))
       
        # Evaluates the density of passengers in a 2 cells radius
        clusters = []
        for i, (px, py) in enumerate(grid_passengers):
            ...
            clusters.append({
                "position": (px, py),
                "value": 1 + nearby_count * self.passenger_group_bonus  # Prefers dense groups
            })
       
        return sorted(clusters, key=lambda x: x["value"], reverse=True)  # Sorts by decreasing value
    ...
    def get_grid_position(self, train=None):
        ...
        return (pos_x // self.cell_size, pos_y // self.cell_size)  # Converts coordinates in grid
